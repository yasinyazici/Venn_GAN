...
def generator(z_inp, mode=0, num_modes = 1, sn=False, bn=True, is_training=False, reuse=False,name='gen'):
    with tf.variable_scope(name, reuse=reuse):
        ...
        x = conv(z, channels=g_dim*8, kernel=4, stride=1, pad=3, use_bias=True, sn=sn, scope='fc')
        x = conditioner(x, mode=mode, num_modes=num_modes, dim=g_dim*8, name = 'cond_0')
        
        x = resblock(x, g_dim*8, g_dim*8, sn, 'res_0')        
        x = up_sample(x, scale_factor=2)
        x = conditioner(x, mode=mode, num_modes=num_modes, dim=g_dim*8, name = 'cond_1')
        x = resblock(x, g_dim*8, g_dim*8, sn, 'res_1')        
        x = up_sample(x, scale_factor=2)
        x = conditioner(x, mode=mode, num_modes=num_modes, dim=g_dim*8, name = 'cond_2')
        x = resblock(x, g_dim*4, g_dim*4, sn, 'res_2')        
        x = up_sample(x, scale_factor=2)
        x = conditioner(x, mode=mode, num_modes=num_modes, dim=g_dim*4, name = 'cond_3')
        x = resblock(x, g_dim*2, g_dim*2, sn, 'res_3')
        x = up_sample(x, scale_factor=2)
        x = conditioner(x, mode=mode, num_modes=num_modes, dim=g_dim*2, name = 'cond_4')
        x = resblock(x, g_dim*1, g_dim*1, sn, 'res_4')
...
def discriminator(x_inp, num_agent=1, multi_head=False, sn = False, reuse=False, name='disc'):
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        x = resblock(x, d_dim*1, d_dim*1, sn, 'res_0')
        x = down_sample(x)
        x = resblock(x, d_dim*1, d_dim*2, sn, 'res_1')
        x = down_sample(x)
        x = resblock(x, d_dim*2, d_dim*4, sn, 'res_2')
        x = down_sample(x)
        x = resblock(x, d_dim*4, d_dim*8, sn, 'res_3')
        x = down_sample(x)
        x = resblock(x, d_dim*8, d_dim*8, sn, 'res_4')
...
def resblock(x, hid_dim, out_dim, sn, name):
